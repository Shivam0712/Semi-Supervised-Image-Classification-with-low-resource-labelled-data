...
class FeatureClassificationModel(Algorithm):
    ...
    def process_batch(self, batch, do_train=True):
        ...
        #***************** SET TORCH VARIABLES ******************
        #dataX_var = Variable(dataX, volatile=((not do_train) or (not finetune_feat_extractor)))#urwa
        if ((not do_train) or (not finetune_feat_extractor)):
            with torch.no_grad():
                dataX_var = Variable(dataX)
        else:
            dataX_var = Variable(dataX)
        ...
        #************ FORWARD PROPAGATION ***********************
        #print('out_feat_keys',len(out_feat_keys))
        feat_var = self.networks['feat_extractor'](dataX_var, out_feat_keys=out_feat_keys)
        #print('feat_var',len(feat_var))
        #if len(out_feat_keys) == 1:
        ...
        if not finetune_feat_extractor:
            if isinstance(feat_var, (list, tuple)):
                for i in range(len(feat_var)):
                    if (not do_train):
                        with torch.no_grad():
                            feat_var[i] = Variable(feat_var[i].data)
                    else:
                        feat_var[i] = Variable(feat_var[i].data)
            else:
                if (not do_train):
                    with torch.no_grad():
                        feat_var = Variable(feat_var.data)
                else:
                    feat_var = Variable(feat_var.data)
        #print(feat_var[0].shape)
