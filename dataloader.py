...
_CUSTOM_DATASET_DIR = '/scratch/um367/DL/data/ssl_data_96/'
#_CUSTOM_DATASET_DIR = '/scratch/um367/DL/data/sample_4/'
_SAMPLE_DATASET_DIR = '/scratch/um367/DL/data/sampledata/'
#_SAMPLE_DATASET_DIR = '/home/urwa/Documents/Courses/DL/Project/data/sampledata/supervised'
...
def subsetSample(samples, maxClassCount):
    labelCount = {}
    subsetsamples = []
    for sample in samples:
        ...
        if label not in labelCount:
            labelCount[label] = 0
        labelCount[label]+=1
        if labelCount[label] <= maxClassCount:
            subsetsamples.append(sample)
...
class GenericDataset(data.Dataset):
    def __init__(self, dataset_name, split, random_sized_crop=False,
        ...
        if self.dataset_name=='sample':
            ...
            split_data_dir = _SAMPLE_DATASET_DIR + '/' + self.split
    ...
    def _keep_first_k_examples_per_category(self, num_imgs_per_cat):
        ...
        if self.dataset_name in ['custom','sample'] :
            self.data.samples = subsetSample(self.data.samples, num_imgs_per_cat)
